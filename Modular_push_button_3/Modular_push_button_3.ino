...
if(count==3 ){
  if(printline==true){ //makes serial input the line only once.
    ...
    if ( i < sizeof(ledPins) - 1 ) {
      ...
      // Turn on the next led. notice that ++ before i increments and returns the new value to the if statement.
      ...
      //  "the starting point of an array will be at the last led that was turned off. I did not bother making it start from 1-5." 
      // in this version i did bother, now it always starts at first led.
...
printline=false;}
  ...
  void mode6(){
   ...
   int n = random(0, 5);  //  Random Integer from 0 to 5

 analogWrite(LEDpins2[n],potwrite);
    }
       if (millis() - blinkMillis2 > blinkInterval4) { //difference in intervals established.
    blinkMillis2 = millis();
     int n = random(0, 5); //  Random Integer from 0 to 5
// if(digitalRead(i)==HIGH){//this will work too, it will set led to 0 when the i is incremented, the problem was that this time can not be cut down, delay cant be used.
